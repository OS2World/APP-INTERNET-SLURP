 ...
 *
 *
 * $Id: slurp.c,v 1.8 1993/08/20 10:34:50 root Exp root $
 *
 * $Log: slurp.c,v $
...
parse_args (int argc, char **argv)
	...
	while ((c = getopt (argc, argv, "a:g:t:h:dilrwfskv")) != EOF)
		switch (c)
			{
			...
			case 'w':	/* Don't set next time */
				no_time_flag++;
				break;
			case 'f':	/* Feed into rnews */
				rnews = TRUE;
				break;
			case 's':	/* Do overlapping NNTP commands */
				speedup_flag++;
				break;
			case 'v':	/* Verbose - print percentage */
				verbose_flag++;
				break;
			case 'k':	/* Keep long filenames for time */
				keep_flag++;
				break;
			default:
				return (1);
...
get_ntime ()
	...
	if (timefile == NULL)
		...
		if (!keep_flag) 
			{
			(void) strncpy (tmpname, hostname,8);
			tmpname[8]='\0';
			if (pos = strchr (tmpname, '.'))
				*pos = '\0';
			(void) strcat (filename, tmpname);
			}
		else 
	...
	if (sublist)
		...
		if (!keep_flag) 
			{
			(void) strncpy (tmpname, sublist,3);
			tmpname[3]='\0';
			if (pos = strchr (tmpname, '.'))
				*pos = '\0';
...
main (int argc, char **argv)
	...
	if (sock_init()) {
		printf("Sorry, TCP/IP is not running");
		exit(1);
	}

	strcpy(sysfile,SYSFILE);
	strcpy(timfile,TIMFILE);
	...
	if (parse_args (argc, argv))
		{
		(void) fprintf (stderr, "Usage: %s [-g newsgroups/distribution] [-t time] [-a username/password] [-h histoyfile]\n", pname);
		(void) fprintf (stderr, "       [-s] [-d] [-i] [-l] [-r] [-w] [-k] [-v] server[/sublist][:timefile]\n");
		exit (2);
		}
	...
	if (verbose_flag)
		openlog(pname, LOG_PID | LOG_PERROR, SYSLOG);
	else
	...
	if (open_history ())
		log_sys ("Can't open history file %s", historyfile);
	...
	if (verbose_flag)
		printf("Getting new article IDs:\n");
	get_ids ();
